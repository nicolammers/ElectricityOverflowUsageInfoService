...
namespace ElectricityOverflowUsageInfoService.SmardApi {
    public class SmardApiReader {
        ...
        public async Task<Indices> GetIndicesAsync() {
            try {
                using (HttpClient client = new HttpClient()) {
                    ...
                    HttpResponseMessage response = await client.GetAsync(new Uri(APIGATEWAY + "/4169/DE/index_hour.json"));
        ...
        public async Task<TimeSeries> GetTimeSeriesAsync(double timestamp) {
            try {
                using (HttpClient client = new HttpClient()) {
                    HttpResponseMessage response = await client.GetAsync(new Uri(APIGATEWAY + "/4169/DE/4169_DE_quarterhour_" + timestamp.ToString() + ".json"));
                    string contentString = await response.Content.ReadAsStringAsync();
